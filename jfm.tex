...
\footnote{これが日本語\TeX{}開発コミュニティによる新仕様である；
オリジナルのアスキーによる仕様では、「上位半ワードに漢字コード、
下位半ワードにキャラクタタイプを持つ」というものであった。
確かにp\TeX{}の内部コード（EUC、SJISまたはJIS）の範囲では、
漢字コードの上限は2バイト（というか2バイトに固定）なので問題はない。
しかし、up\TeX{}のように3バイト以上（U+10000以上）の文字を扱う場合は、
オリジナルの仕様では漢字コードを格納できない。}。
...
して、3バイト以上（U+10000以上）の漢字コードで不足する1バイトを確保した。
...
\item{tag(2bits)}
	...
	\item{tag=1} {\it glue\_kern}プログラムが{\it glue\_kern[remainder]}
		からに収められていることを示す。
	\item{tag=2、3} 使用しない。
	\end{description}
