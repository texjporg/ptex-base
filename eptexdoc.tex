...
\end{verbatim}
 ...
 \csitem[\.{pdflastxpos}, \.{pdflastypos}]
  \.{pdfsavepos}\index{\.{pdfsavepos}}が置かれた場所の，dviにおける出力位置を返す内部整数（読み取り専用）．
...
\end{verbatim}
 ...
 \csitem[\.{pdfuniformdeviate} <number>, \.{pdfnormaldeviate}]
  ...
  \.{pdfnormaldeviate}は，平均値0，標準偏差65\,536の正規分布に従う乱数（整数値）を生成する．
  
  現在の乱数生成の種の値は，\.{pdfrandomseed}\index{\.{pdfrandomseed}}で取得できる（読み取り専用）．
  種の初期化にはシステムのマイクロ秒単位での現在時刻情報が使われる．また，種の値は
  \.{pdfsetrandomseed} <number>\index{\.{pdfsetrandomseed}}によって特定の値に設定可能である．
