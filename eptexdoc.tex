...
\section{\.{lastnodechar}プリミティブ}
platex/??において，「最後のグルーがJFMグルーだけ\.{unskip}する」処理の必要性が
述べられた．\epTeX にはもともと最後のノードの種別を返す\.{lastnodetype}プリミティブが
...
subtypeの具体的な数値は将来の\epTeX の仕様変更によって変わる恐れがあるので，
本ドキュメントでは具体的な値は記述しない．
例えば\pLaTeX~2018-04-01以降では，JFMグルーの|\lastnodetype|, |\lastnodesubtype|の値を
\begin{verbatim}
...
\end{verbatim}
のように保存し，「最後のグルーがJFMグルーのときだけ消す」命令|\removejfmglue|を
\begin{verbatim}
...
\begin{curve}
 \TeX82ではカーンのsubtypeは
\begin{itemize}
 \item フォントのカーニングに由来するカーン
 \item |\kern|によって明示的に発行されたか，イタリック補正(|\/|)のカーン
 \item |\accent|の内部処理でアクセントの位置補正のために作られたカーン
\end{itemize}
の3種類に分類しているが，\pTeX 系列ではさらに
「|\kern|によって明示的に発行されたカーン」「イタリック補正(|\/|)のカーン」に
異なるsubtypeを割り当てている．
\end{curve}
